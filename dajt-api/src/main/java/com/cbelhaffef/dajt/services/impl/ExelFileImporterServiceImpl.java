...
import com.cbelhaffef.dajt.dao.enums.FolderColumnImportEnum;
import com.cbelhaffef.dajt.dao.enums.FolderPriority;
...
import static com.cbelhaffef.dajt.dao.enums.FolderTopic.UNKOWN;
import static com.cbelhaffef.dajt.dao.enums.StatusImport.IN_PROGRESS;
import static com.cbelhaffef.dajt.dao.enums.StatusImport.ON_ERROR;
...
public class ExelFileImporterServiceImpl implements ExelFileImporterService {
                ...
                for(int i = 0 ; cellIterator.hasNext() && !isErrorProcessing ; i++){
                    ...
                    if (i == FolderColumnImportEnum.KEY.getValue()) {
                        if (currentCell.getCellTypeEnum() == CellType.BLANK) {
                            log.warn("la ligne " + count + " du fichier d'import " + fileExel.getName() + " à été ignorée car le numéro du dossier est absent");
    ...
    public void endImportOnError(File file, Import imp, String message) {
        String msg = "L'import du fichier : " + imp.getFileName() +" a échoué - " + message;
        log.error(msg);
